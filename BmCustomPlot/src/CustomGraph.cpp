    ...
    : QDialog(parent)
{
    ...
    connect(m_btnOK, &QPushButton::clicked, this, &CoordInputDialog::onBtnOkClicked);
    connect(m_spinBoxX, SIGNAL(valueChanged(double)), this, SLOT(setCoordinateX(double)));
    connect(m_spinBoxY, SIGNAL(valueChanged(double)), this, SLOT(setCoordinateY(double)));
}

CoordInputDialog::~CoordInputDialog() = default;

void CoordInputDialog::initUi()
{
    auto *xLabel = new QLabel("X:", this);
    m_spinBoxX = new QDoubleSpinBox(this);
    m_spinBoxX->setSingleStep(0.01);
    auto *yLabel = new QLabel("Y:", this);
    m_spinBoxY = new QDoubleSpinBox(this);
    m_spinBoxY->setSingleStep(0.01);
    m_btnOK = new QPushButton("OK", this);

    auto *layout = new QGridLayout(this);
    layout->addWidget(xLabel, 0, 0, 1, 1);
    layout->addWidget(m_spinBoxX, 0, 1, 1, 2);
    layout->addWidget(yLabel, 1, 0, 1, 1);
    layout->addWidget(m_spinBoxY, 1, 1, 1, 2);
